...
# —————— Configuración de la salida safe_print ——————
...
usuario = config_runtime.usuario
# reporte puede venir sin extensión
reporte = config_runtime.reporte.strip()
if not reporte.lower().endswith(".pbit"):
    reporte += ".pbit"

base_report_directory = config.base_report_directory

# —————— Buscar el archivo real en la carpeta ——————
base_name = os.path.splitext(reporte)[0]
candidatos = {
    f"{base_name} + {usuario}.pbit",
    f"{usuario} + {base_name}.pbit"
}

found_file = None
for f in os.listdir(base_report_directory):
    if f in candidatos:
        found_file = f
        break

if not found_file:
    safe_print(f"❌ No encontré ningún .pbit en {base_report_directory} que sea:\n  • {base_name} + {usuario}.pbit\n  • {usuario} + {base_name}.pbit")
    safe_print("Archivos .pbit disponibles:")
    for f in os.listdir(base_report_directory):
        if f.lower().endswith(".pbit"):
            safe_print(f"  – {f}")
    sys.exit(1)

pbit_file_path = os.path.join(base_report_directory, found_file)
safe_print(f"📊 Analizando el reporte real: {found_file}  | Usuario: {usuario}")

# —————— Construir y lanzar Power BI ——————
powerbi_exe_path = config.powerbi_exe_path
command_line = f'"{powerbi_exe_path}" "{pbit_file_path}"'

safe_print("🟡 Lanzando Power BI...")
app = Application(backend="uia").start(command_line)
safe_print("🟢 Power BI lanzado.")
time.sleep(10)

# —————— Maximizar ventana de Power BI ——————
try:
    safe_print("🔍 Buscando ventana principal de Power BI para maximizar...")
    max_window = None
    for _ in range(10):
    ...
    if max_window:
        max_window.set_focus()
        max_window.maximize()
        safe_print("🟢 Power BI maximizado correctamente.")
    else:
        safe_print("⚠️ No se encontró la ventana principal de Power BI para maximizar.")
except Exception as e:
    safe_print(f"❌ Error al maximizar Power BI: {e}")

time.sleep(1.5)
pyautogui.click(725, 1201)
